...
function getPaths(publicPath, compiler, url) {
  ...
  // :bm, multi-compiler 这个case比较极端, 基本碰不到, 所以看得时候可以简单忽略
  if (Array.isArray(compilers)) {
    ...
    for (let i = 0; i < compilers.length; i++) {
      ...
      if (compilerPublicPath) {
        ...
        // :bm, find first match on public path, then return eagerly
...
module.exports = {
  // :bm, return false: if the case is not supported, not a error case
  getFilenameFromUrl(pubPath, compiler, url) {
    ...
    //:bm, why url can have a null hostname case, doesnt make sense to me
    if (
      !urlObject.hostname &&
